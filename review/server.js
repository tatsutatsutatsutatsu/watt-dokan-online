...
function bumpStat(seasonKey, kind, difficulty, pid, win, ranked) {
  ...
  // ranked* がない既存レコードは、これまでの対戦がすべてガチモードだったものとして移行する。
  if (!Number.isInteger(rec.rankedGames)) {
    rec.rankedWin = rec.win || 0;
    rec.rankedLoss = rec.loss || 0;
    rec.rankedGames = rec.games || 0;
  }
  rec.games++;
...
function computeTop(seasonKey, kind, difficulty, limit) {
  ...
  const arr = Object.entries(bucket).map(([pid, rec]) => {
    const games = Number.isInteger(rec.rankedGames) ? rec.rankedGames : (rec.games || 0);
    const win = Number.isInteger(rec.rankedWin) ? rec.rankedWin : (rec.win || 0);
    const loss = Number.isInteger(rec.rankedLoss) ? rec.rankedLoss : (rec.loss || 0);
    return {
      pid, name: (stats.players[pid] || {}).name || "???",
      win, loss, games, winRate: games ? win / games : 0,
...
function recordMatchResult(room, winnerSeat) {
  ...
  const ranked = room.mode === "gachi"; // enjoyも総戦績には残すが、月間順位には加算しない
  if (room.cpu) {
    const seat = room.seats.A;
    if (seat && seat.playerId) bumpStat(seasonKey, "cpu", room.difficulty, seat.playerId, winnerSeat === "A", ranked);
  } else {
    ["A", "B"].forEach(seatX => {
      const seat = room.seats[seatX];
      if (seat && seat.playerId) bumpStat(seasonKey, "pvp", null, seat.playerId, winnerSeat === seatX, ranked);
    });
  }
  saveStatsDebounced();
}
...
/* ==================== 静的ファイル配信 (public/) ==================== */
const MIME = { ".html": "text/html; charset=utf-8", ".js": "text/javascript", ".css": "text/css", ".png": "image/png", ".wav": "audio/wav", ".json": "application/json; charset=utf-8", ".webmanifest": "application/manifest+json; charset=utf-8" };
...
const POOL = [
 ...
 {id:"coal_boiler",n:"石炭ボイラー",e:"🔥",c:400,t:"f",a:3,h:3,fx:"buffthermal1atk",tx:"ファンファーレ：味方の火力フォロワー全体を+1/+0",type:"火力"},
 {id:"gas_turbine",n:"ガスタービン",e:"🌪️",c:600,t:"f",a:5,h:3,kw:"疾走",tx:"疾走：設置ターンからリーダーも攻撃可",type:"火力"},
 {id:"thermal_plant",n:"火力発電所",e:"🏭",c:800,t:"f",a:5,h:5,fx:"thermalface",tx:"ファンファーレ：味方の火力フォロワーの数だけ敵リーダーに1ダメージ",type:"火力"},
 {id:"boiler_pressure",n:"ボイラー圧力上昇",e:"♨️",c:300,t:"s",fx:"buffthermal21",tg:"ally",tx:"味方の火力フォロワー1体を+2/+1",type:"火力"},
 {id:"chimney",n:"煙突",e:"🏭",c:200,t:"f",a:1,h:3,kw:"守護",lw:"searchthermal",tx:"守護／ラストワード：山札から火力フォロワーを1枚探して手札に加える（なければ1枚ドロー）",type:"火力"},
 {id:"waterwheel",n:"水車",e:"☸️",c:200,t:"f",a:1,h:2,turnEnd:"heal1",tx:"ターン終了時：自リーダーを1回復",type:"水力"},
 {id:"dam",n:"ダム",e:"🌊",c:700,t:"f",a:3,h:7,kw:"守護",tx:"守護：先に攻撃を受け止める",type:"水力"},
 {id:"pumped_storage",n:"揚水発電所",e:"🔄",c:500,t:"f",a:3,h:4,kw:"ラストワード",lw:"returnonce",tx:"ラストワード：このカードを手札に戻す（1回限り）",type:"水力"},
 {id:"dam_release",n:"ダム放流",e:"💦",c:400,t:"s",fx:"hydroaoe",tx:"味方の水力フォロワーの数だけ敵フォロワー全体に1ダメージ",type:"水力"},
 {id:"hydro_plant",n:"水力発電所",e:"💧",c:800,t:"f",a:4,h:6,kw:"ドレイン",fx:"heal3",tx:"ドレイン／ファンファーレ：自リーダーを3回復",type:"水力"},
];
...
const TOKEN_OUTLET = { id:"outlet", n:"コンセント", e:"🔌", c:0, t:"f", a:1, h:1, tx:"", type:"設備" };
const MAXW = 1000, BOARD_MAX = 4, HAND_MAX = 8;
const EVO = { A:{turn:5,ep:2}, B:{turn:4,ep:3} }; // 先攻:5T/EP2 後攻:4T/EP3

/* ==================== デッキ構築 ==================== */
const DECK_SIZE = 20;
/* デフォルトデッキ：40種のカードプールから両種族シナジーを含む20枚を採用。不正デッキの代替として使用。 */
const DEFAULT_DECK = [
  "led","fuse","ground","short","wiring",
  "coal_boiler","gas_turbine","thermal_plant","boiler_pressure","chimney",
  "waterwheel","dam","pumped_storage","dam_release","hydro_plant",
  "washer","microwave","generator","charge_station","panelboard",
];
...
function randomValidDeck() {
  const bag = POOL.flatMap(c => [c.id, c.id]); // 40種を2枚ずつ入れたバッグ
  shuffle(bag);
...
function fanfare(G, s, e, card) {
  ...
  if (card.fx === "buffthermal1atk") s.board.filter(u => u.type === "火力").forEach(u => u.a += 1);
  if (card.fx === "thermalface") e.hp -= s.board.filter(u => u.type === "火力").length;
  if (card.fx === "hydroaoe") {
    const n = s.board.filter(u => u.type === "水力").length;
    e.board.forEach(u => u.hp -= n);
  }
  if (card.fx === "tacoashi") {
...
function triggerLastword(G, seatX, u) {
  ...
  else if (u.lw === "searchthermal") {
    const idx = s.deck.findIndex(c => c.t === "f" && c.type === "火力");
    if (idx >= 0) {
      const [found] = s.deck.splice(idx, 1); // サーチでは山札をシャッフルしない
      if (s.hand.length < HAND_MAX) s.hand.push(found);
      setMsg(G, `${G.names[seatX]}：${u.n}のラストワード発動！💀 火力フォロワーをサーチ`);
    } else {
      drawN(G, seatX, 1);
      setMsg(G, `${G.names[seatX]}：${u.n}のラストワード発動！💀 火力フォロワーがないため1枚ドロー`);
    }
  }
  else if (u.lw === "returnonce" && !u.returnUsed) {
    if (s.hand.length < HAND_MAX) {
      s.hand.push({ ...POOL_BY_ID.get(u.cid), returnUsed:true });
      setMsg(G, `${G.names[seatX]}：${u.n}のラストワード発動！💀 手札に戻った`);
    } else {
      setMsg(G, `${G.names[seatX]}：${u.n}のラストワード発動！💀 手札上限のため消滅`);
    }
  }
}
function processDeaths(room) {
  ...
  while (guard++ < 50) {
    ...
    dead.forEach(({ u, seatX }) => { if (u.lw) { triggerLastword(G, seatX, u); triggered.push({ id:"u"+u.id, lw:u.lw }); } });
  }
  return triggered;
}

function triggerTurnEnd(G, seatX) {
  const s = G.S[seatX];
  const heal = s.board.filter(u => u.turnEnd === "heal1").length;
  if (heal > 0) {
    const before = s.hp;
    s.hp = Math.min(20, s.hp + heal);
    if (s.hp > before) setMsg(G, `${G.names[seatX]}：水車の効果で${s.hp - before}回復`);
  }
}

/* HPの値そのものではなく「10以下ゾーンへの進入」を記録する。
   11以上へ回復した時点でfalseへ戻るため、次の進入時に再び発動できる。 */
function checkEmergency(room, rng) {
  ...
  ["A", "B"].forEach(seatX => {
    ...
    if (random() < 0.5) {
      ...
      effect = "draw";
      effectName = "🔋バックアップバッテリー";
      drawN(G, seatX, 1);
    }
    setMsg(G, `${G.names[seatX]}：非常用電源が起動（${effectName}を発動）`);
...
function newGame(room, nameA, nameB) {
  ...
  const G = {
    ...
    emergencyLow: { A:false, B:false }, // trueの間は10以下ゾーン内。11以上へ戻ると再武装
    statsRecorded: false,
...
function applyAction(room, seatX, act) {
  ...
  if (act.a === "play" && Number.isInteger(act.hand)) {
    ...
    if (card.t === "f") {
      ...
      const targetBoard = card.tg === "ally" ? me.board : op.board;
      const t = targetBoard[act.ti]; if (!t) return;
      if (card.fx === "buffthermal21" && t.type !== "火力") return;
      me.w -= card.c; me.hand.splice(act.hand,1);
      if (card.fx === "dmg3") t.hp -= 3;
      if (card.fx === "kill") t.hp = 0;
      if (card.fx === "buffthermal21") { t.a += 2; t.hp += 1; t.maxhp += 1; }
      setMsg(G, `${nm}：${card.n} → ${t.n}！`);
      evType = "spell"; evMeta = { seat:seatX, cardName:card.n, targetUnitId:t.id, targetSeat:card.tg === "ally" ? seatX : other(seatX) };
    } else {
  ...
  const { diffs, deaths } = diffHP(before, G);
  if (evType) pushEvent(G, { type:evType, diffs, deaths, lastwords, emergencies, ...evMeta });
  broadcast(room);
...
/* ==================== CPU（ソロモード） ==================== */
function spellTargetIndexes(card, S, E) {
  if (!card.tg) return [];
  const board = card.tg === "ally" ? S.board : E.board;
  return board.map((u, i) => ({ u, i }))
    .filter(({ u }) => card.fx !== "buffthermal21" || u.type === "火力")
    .map(({ i }) => i);
}
...
function normalPick(S, E) {
  const cands = S.hand.map((c,i) => ({c,i}))
    ...
    .filter(x => !x.c.tg || spellTargetIndexes(x.c, S, E).length)
  ...
  let ti;
  if (pick.c.tg === "ally") {
    ti = spellTargetIndexes(pick.c, S, E)[0];
  } else if (pick.c.tg) {
...
function easyPick(S, E) {
  ...
  let ti;
  if (pick.c.tg) {
    const targets = spellTargetIndexes(pick.c, S, E);
    ti = targets[Math.floor(Math.random()*targets.length)];
  }
  return { i: pick.i, ti };
}
...
function hardPick(S, E) {
  ...
  if (pick.c.tg === "ally") {
    const targets = spellTargetIndexes(pick.c, S, E);
    ti = targets[0];
    targets.forEach(j => { if ((S.board[j].a + S.board[j].hp) > (S.board[ti].a + S.board[ti].hp)) ti = j; });
  } else if (pick.c.tg) {
    if (pick.c.fx === "kill") {
...
wss.on("connection", (ws) => {
  ws.on("message", (raw) => {
    ...
    if (m.type === "create") {
      const code = genCode();
      const room = makeRoom(code, false, m.mode);
      ...
      send(ws, { type:"created", code, token, mode:room.mode });
    }
      ...
      // join側がmodeを送っても参照せず、作成者が固定したroom.modeへ合流させる。
      send(ws, { type:"joined", code: room.code, token, mode:room.mode });
      ...
      const code = genCode();
      const room = makeRoom(code, true, m.mode);
      ...
      rooms.set(code, room);
      ws.roomCode = code; ws.seat = "A";
      send(ws, { type:"created", code, token, solo:true, mode:room.mode });
      newGame(room, room.seats.A.name, room.seats.B.name);
    }

    else if (m.type === "tutorial") {
      const code = genCode();
      const room = makeRoom(code, true, "gachi");
      room.tutorial = true;
      const token = genToken();
      ...
      send(ws, { type:"resumed", code: room.code, waiting: !room.G, mode:room.mode });
      const oppSeat = room.seats[other(seatX)];
...
/* 外部APIではなく、依存追加なしの回帰テストからゲームエンジンを検証するための限定公開。 */
if (process.env.NODE_ENV === "test") {
  module.exports = {
    POOL, POOL_BY_ID, DEFAULT_DECK, TOKEN_OUTLET, HAND_MAX,
    mkSide, summon, fanfare, triggerLastword, processDeaths, triggerTurnEnd, applyAction,
    spellTargetIndexes, normalPick, easyPick, hardPick,
    normMode, makeRoom, newGame, buildSnap, checkEmergency,
    stats, bumpStat, computeTop, recordMatchResult,
  };
}
